                ...
                {CATEGORIES.map(cat => (
                  <label key={cat} className="flex items-center space-x-2 cursor-pointer group">
                    <input 
                      ...
                      className="h-4 w-4 text-slate-900 focus:ring-slate-500 border-gray-300 rounded transition-colors"
                    />
                    <span className="text-sm text-gray-700 group-hover:text-slate-900 transition-colors">{cat}</span>
                  </label>
                ))}
              </div>
            </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {Array.isArray(product.category) ? (
                            product.category.map((cat: string) => (
                              <span key={cat} className="bg-slate-100 px-2 py-0.5 rounded text-[10px] font-medium">{cat}</span>
                            ))
                          ) : (
                            <span className="bg-slate-100 px-2 py-0.5 rounded text-[10px] font-medium">{product.category || 'Uncategorized'}</span>
                          )}
                        </div>
                      </td>
